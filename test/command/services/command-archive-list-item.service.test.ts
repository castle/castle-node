import { CommandArchiveListItemService } from '../../../src/command/command.module';
import { Configuration } from '../../../src/configuration';
import AbortController from 'abort-controller';
import MockDate from 'mockdate';
import { InvalidParametersError } from '../../../src/errors';

describe('CommandArchiveListItemService', () => {
  beforeEach(() => {
  ...
  describe('call', () => {
    ...
    const expected = {
      requestUrl: new URL(
        `https://castle.io/v1/lists/${options.list_id}/items/${options.id}/archive`
      ),
      requestOptions: {
    ...
    it('generates payload', () => {
      const received = CommandArchiveListItemService.call(
        controller,
        options,
        config
      );
    ...
    test.each(['list_id', 'id'])(
      ...
      (prop) => {
        ...
        expect(() =>
          CommandArchiveListItemService.call(controller, invalidOptions, config)
